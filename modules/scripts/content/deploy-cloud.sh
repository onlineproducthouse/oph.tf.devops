...
if [[ "$LOAD_ENV_VARS_SCRIPT_S3_URL" == "" ]];then
  echo "[deploy-cloud]: load-env-vars script AWS S3 URL not set. please set load-env-vars script AWS S3 URL"
  exit 1
fi

if [[ "$ENV_VARS_S3_URL" == "" ]];then
  echo "[deploy-cloud]: .env file AWS S3 URL not set. please set .env file AWS S3 URL"
  exit 1
fi
